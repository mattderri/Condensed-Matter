...
\subsection{Polyacetylene Bands}
...
\left\{
\begin{aligned}
&\bra{\phi_A(x)}[H_{\text{at}}+\Delta U(x)]\ket{\psi_k(x)}=\bra{\phi_A(x)}\varepsilon_k\ket{\psi_{\Vec{k}}(x)}\\
...
=&E_AC_A
\end{align*}
Of the final six terms, only the first one survives because the other ones involve the product of wave functions centered at different sites, hence they are all equal to zero. Let's look now at the term with the potential:
\begin{align*}
...
\]
Repeating exactly the same arguments for the projection on $\phi_B(x-a_0)$ we end up with:
\[
