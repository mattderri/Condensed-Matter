...
\end{figure}\\
\noindent
Given its electronic configuration, there are $sp^2$ orbitals in the plane and a $p_z$ orbital left free. The idea here is to decouple the $\sigma$-bands [$sp^2$] and the $p_z$ bands. The wave function is given by:
\[
...
\begin{figure}[h]
    \centering
    \includegraphics{polyscheme.pdf}
...
We move now to study the graphene band structure, whose orbital configuration is displayed in the picture below.
\begin{figure}[h]
    \centering
...
\end{figure}\\
\noindent
In the plane, we can see this schematically as a honeycomb lattice with a basis.
\begin{figure}[h]
    \centering
...
\end{figure}\\
\noindent
The vectors $\Vec{a_1}$ and $\Vec{a_2}$ are defined as:
\[
\Vec{a_1}=\frac{a}{2}(\sqrt{3},1) \qquad \Vec{a_2}=\frac{a}{2}(\sqrt{3},-1)
\]
Things are more complicated than the previous case since now we are working with coordinates on a plane, instead of considering just a 1D lattice. The wave function in this case is given by:
\[
...
The solution is:
\[
\varepsilon_{\Vec{k}}=\frac{E_A+E_B\pm\sqrt{(E_A-E_B)^2+4\gamma^2\left[1+4\cos\left(\frac{k_xa\sqrt{3}}{2}\right)\cos\left(\frac{k_ya}{2}\right)+4\cos^2\left(\frac{k_ya}{2}\right)\right]}}{2}
\]
...
\end{wrapfigure}
Check now the behaviour of this dispersion band in the \nth{1} Brillouin zone: for a 2D hexagonal lattice, the \nth{1} Brillouin zone corresponds to another hexagonal rotated by 90$^\circ$ of side $b=4\pi\sqrt{3}/9l$, where $l$ is the side of the hexagonal lattice. In our case, this corresponds to:
\[
